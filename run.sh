...
#   ./run.sh shell              # コンテナ内シェルに入る (デバッグ用)
...
case "${1:-}" in
  ...
  web-init)
    # Next.js プロジェクトの雛形を ./web に作る (初回のみ)
    if [ -d "$SCRIPT_DIR/web" ] && [ "$(ls -A "$SCRIPT_DIR/web" 2>/dev/null)" ]; then
      echo "Error: ./web is not empty. Remove it first if you want to re-init."
      exit 1
    fi
  ...
  web)
    ...
    if [ ! -f "$SCRIPT_DIR/web/package.json" ]; then
      echo "Error: ./web/package.json not found. Run '$0 web-init' first."
      exit 1
    fi
    docker run --rm -it \
      --name stock-web \
  ...
  *)
    ...
    echo ""
    echo "  build              Build Docker image (Python worker)"
    ...
    echo "  shell              Open shell inside Python worker (debug)"
    echo "  web-init           Bootstrap Next.js scaffold into ./web (run once)"
